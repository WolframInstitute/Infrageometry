...
BallVolumes::usage = "BallVolumes[g, v] gives the List {V(0), ..., V(ecc(v))} of cumulative ball volumes V(r) = |B_r(v)| (position i is radius i - 1). Slot 2 is a single vertex, a list, or All (default); slot 3 selects radii: r_Integer returns the scalar V(r), {rmin, rmax} a rectangular window (saturating past eccentricity, so a fixed window over a vertex list Transposes for subset statistics), All (default) the full profile. Option \"Ball\" picks the counting convention (\"Closed\" (default) = |B_r| | \"Peeled\" = |B_{r-1}| | \"Interior\" = |B_r| - |dB_r|). The growth invariant underlying VolumeGrowthParameters; BallVolumes[\"Closed\"] == Accumulate[ShellAreas].";
...
VolumeGrowthParameters::usage = "VolumeGrowthParameters[g, v, window] returns the fitted growth parameters at vertex v as the nested association <|\"Ball\" -> <|\"Dimension\" -> d, \"ScalarCurvature\" -> R, \"CurvatureByRadius\" -> {R(1), ...}, \"Window\" -> {rmin, rmax}|>, \"Sphere\" -> <|\"Dimension\" -> n, \"ScalarCurvature\" -> S, \"CurvatureByRadius\" -> {S(1), ...}, \"MeanCurvatureByRadius\" -> {H(1), ...}, \"Window\" -> {rmin, rmax}|>|>, fitting BOTH growth probes by Bishop-Gromov regression of the log-difference quotient q(r) on r (r + 1). The ball (volume V(r)) intercept is the dimension d with R = -3 (d + 2) slope; the sphere (area A(r) = ShellAreas, Gray's Area(S_r) = sigma_{n-1} r^(n-1) (1 - S/(6 n) r^2)) intercept is n - 1, reported as the manifold dimension n = intercept + 1 with S = -3 n slope. \"CurvatureByRadius\" is the per-radius comparison profile (ball 6 (d + 2)/r^2 (1 - V/V_E); sphere 6 n/r^2 (1 - A/A_E)); \"MeanCurvatureByRadius\" is the discrete geodesic-sphere mean curvature d Log A/dr. The two probes give independent (n, S) readouts, so their agreement is a consistency check. The window is {rmin, rmax}, All, or Automatic (default), the linear core of the (x, q) scatter (the sphere's Automatic window uses the rising part of A(r) only, since A is non-monotonic on a finite graph). VolumeGrowthParameters[g, verts, window] returns one association per vertex and VolumeGrowthParameters[g] (= VolumeGrowthParameters[g, All]) one per VertexList[g]. Options: \"Dimension\" (Automatic (default) | Integer, pins the intercept) and \"Ball\" (\"Peeled\" (default) | \"Closed\" | \"Interior\").";
...
FindBallCover::usage = "FindBallCover[g, r] returns a minimum r-ball cover of g: a smallest set of centres whose radius-r balls cover every vertex (a minimum r-dominating set). FindBallCover[g, r, targets] covers only the given vertex subset (centres still chosen from all of g). FindBallCover[g, r, targets, count] returns up to count distinct minimum covers for an integer count or UpTo[count], or every one for All; count defaults to 1 (a single cover).";
...
SierpinskiGraph::usage = "SierpinskiGraph[n, k] is the Sierpinski graph S(n, k) on length-n strings over {0, ..., k-1} (k defaults to 3); the corner-gluing family, with S(n, 3) the Tower-of-Hanoi graph on n disks. Laid out on a regular k-gon so it draws as the gasket. Distinct from the vertex-sharing gasket sieve graph GraphData[{\"SierpinskiGasket\", n}].";
FractalCellGraph::usage = "FractalCellGraph[survivors, n] is the level-n cell-adjacency graph of the IFS keeping the sub-cells survivors (offset vectors in {0,...,b-1}^d) of the b^d grid; cells are adjacent iff face-adjacent and vertices are their integer grid coordinates. MengerGraph and SierpinskiCarpetGraph are presets.";
MengerGraph::usage = "MengerGraph[n] is the level-n Menger sponge skeleton (the 20 sub-cubes of the 3x3x3 grid with fewer than two coordinates equal to 1); 20^n vertices. View with Graph3D[MengerGraph[n]].";
SierpinskiCarpetGraph::usage = "SierpinskiCarpetGraph[n] is the level-n Sierpinski carpet cell-adjacency graph (the 8 sub-squares of the 3x3 grid minus the center); 8^n vertices, level 1 the 8-cycle.";
