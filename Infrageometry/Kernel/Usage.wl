...
WolframRicciCurvature::usage = "WolframRicciCurvature[g, v, r] returns the volume-comparison Ricci scalar R(v, r) at vertex v; the radius slot is a selector, so a span {rmin, rmax} or All (default) returns the list of R(v, r) over that window (Mean it for a single scalar). WolframRicciCurvature[g, verts, range] returns a list over the given vertices, and WolframRicciCurvature[g, All, range] (= WolframRicciCurvature[g]) a list over VertexList[g]. Options: \"Dimension\" (Automatic (default) | Integer) and, when Automatic, \"Differencing\" (\"Forward\" (default), \"Backward\", \"Central\", \"Trapezoid\", or {\"Stencil\", offsets}) for the finite-difference scheme estimating the local dimension.";
WolframHausdorffDimension::usage = "WolframHausdorffDimension[g, v, r] returns the volume-growth local dimension d(v, r) = d log V / d log r, V(r) = |B_r(v)|, at vertex v; the radius slot is a selector, so a span {rmin, rmax} or All (default) returns the list of d(v, r) over that window (Mean it for a single scalar). WolframHausdorffDimension[g, verts, range] returns a list over the given vertices, and WolframHausdorffDimension[g, All, range] (= WolframHausdorffDimension[g]) a list over VertexList[g]. Option \"Differencing\" (\"Forward\" (default), \"Backward\", \"Central\", \"Trapezoid\", or {\"Stencil\", offsets} for a general k-point stencil) picks the finite-difference scheme; radii outside a scheme's stencil return Indeterminate.";
