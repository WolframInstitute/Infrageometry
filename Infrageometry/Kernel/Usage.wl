...
BoundarylessGraph::usage = "BoundarylessGraph[g] deletes the exterior boundary GraphExteriorBoundary[g] and returns the largest connected component of the subgraph induced on the interior, preserving the original vertex coordinates and their dimension; BoundarylessGraph[mr] does the same on the 1-skeleton of the MeshRegion, with the exact surface as the boundary. Options: Method (passed to GraphExteriorBoundary; Graph form only), \"KeepCoordinates\" (True (default) carries the coordinates over, False drops them).";
