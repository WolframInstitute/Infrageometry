...
InteriorGraph::usage = "InteriorGraph[g] strips the degree-boundary of g and returns the largest connected component, preserving the original vertex coordinates and their dimension. Option Method (\"AverageDegree\" (default, for meshes: deletes every edge both of whose endpoints have below-average degree), \"MaxDegree\" (for lattices: the induced subgraph on full-degree vertices)).";
...
ExampleGraphData::usage = "ExampleGraphData[name, size] is the example substrate name at tier \"Small\", \"Medium\" or \"Large\", read from the frozen table $ExampleGraphs. ExampleGraphData[name, spec] rebuilds it live for a raw spec (a cell measure for \"Plane\", a radius for a tiling). ExampleGraphData[] lists the names; ExampleGraphData[name] gives all three tiers. Graph options are forwarded.";
$ExampleGraphs::usage = "$ExampleGraphs is a Dataset of the example substrates shipped with the paclet, keyed by name and by tier, so that figures reproduce independently of mesh and layout randomness.";
...
InfraSubstrate::usage = "InfraSubstrate[name, size] is the named example substrate generated on demand at tier \"Small\", \"Medium\" or \"Large\", or at a raw spec (a cell measure, a radius, grid dimensions, a generation count); generation is seeded per (name, spec), so repeated calls return the same graph. A Wolfram-model universe is named by its Registry of Notable Universes number, as in InfraSubstrate[\"wm6655\", 11], and any of the 947 registry entries resolves through ResourceFunction[\"WolframModelData\"]. InfraSubstrate[name, size, style] overrides the ambient style, which defaults to the AmbientGraphStyle gray of that tier since a substrate is a backdrop; pass \"Default\" for none. A substrate whose vertices sit somewhere -- a mesh, a lattice, a torus, a surface of revolution, a fullerene -- is drawn where it lives; option \"KeepCoordinates\" -> False discards that embedding and lets the graph layout place the vertices instead. A patch of an unbounded or closed geometry -- a plane mesh, a grid, a tiling disk -- is delivered without its cut rim: option \"Interior\" -> Automatic (default) peels one boundary layer on the patch substrates and leaves intrinsic or absent boundaries alone; False keeps the full patch, and an integer n keeps only the vertices at graph distance at least n from the rim. InfraSubstrate[] lists the names; InfraSubstrate[name] is the \"Medium\" tier. Graph options are forwarded.";
