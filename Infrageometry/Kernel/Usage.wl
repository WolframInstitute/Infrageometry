...
InfraSubstrate::usage = "InfraSubstrate[name, size] is the named example substrate generated on demand at tier \"Small\", \"Medium\" or \"Large\", or at a raw spec (a cell measure, a radius, grid dimensions, a generation count); generation is seeded per (name, spec), so repeated calls return the same graph. A Wolfram-model universe is named by its Registry of Notable Universes number, as in InfraSubstrate[\"wm6655\", 11], and any of the 947 registry entries resolves through ResourceFunction[\"WolframModelData\"]. InfraSubstrate[name, size, style] overrides the ambient style, which defaults to the AmbientGraphStyle gray of that tier since a substrate is a backdrop; pass \"Default\" for none. A substrate is bare combinatorics by default -- a stored embedding (a mesh, a lattice, a torus, a fullerene) is discarded and a spring layout of its own dimension places the vertices; option \"KeepCoordinates\" -> True draws the substrate where it lives instead. A patch of an unbounded or closed geometry -- a plane mesh, a grid, a tiling disk -- is delivered without its cut rim: option \"Interior\" -> Automatic (default) strips the rim of the patch substrates via InteriorGraph, so no closed rim contour remains, and leaves intrinsic or absent boundaries alone; False keeps the full patch, and an integer n peels n - 1 vertex layers before the strip. InfraSubstrate[] lists the names; InfraSubstrate[name] is the \"Medium\" tier. Graph options are forwarded.";
