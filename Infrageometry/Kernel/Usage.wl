...
InfraSubstrateCode::usage = "InfraSubstrateCode[name, size, style] is the code behind InfraSubstrate[name, size, style]: one self-contained expression, as a string, that evaluates to the identical graph. The roster line is read off its own definition rather than transcribed, so the printed code cannot drift from the code that runs; the size table, the generation seed and the layout dimension are baked in, and the private helpers the line calls ride along as local definitions. Takes the options of InfraSubstrate, and Graph options are forwarded as they are by InfraSubstrate.";
