...
TessellationNeighborhoodGraph::usage = "TessellationNeighborhoodGraph[{p, q}, r] is the radius-r graph-distance ball cut from the infinite regular {p, q} tessellation of its covering surface (the non-compact companion to TessellationGraph), grown in concentric reflection rings and dispatched by curvature (p-2)(q-2): the Euclidean plane (== 4), the hyperbolic plane (> 4), or the closing-up sphere (< 4). Interior vertices have degree q; VertexCoordinates carry the embedding (2-D in the plane / Poincare disk, 3-D on the unit sphere). TessellationNeighborhoodGraph[{p, q}, {m, n}] is the m x n rectangular Euclidean patch. TessellationNeighborhoodGraph[config, r] takes a longer vertex configuration (a uniform / Archimedean tiling) and cuts the same ball, grown one corona at a time (the Euclidean uniform tilings 3.6.3.6, 3.4.6.4, 4.6.12, 4.8.8, 3.12.12 and the spherical Archimedean solids; snub and hyperbolic uniform families are deferred).";
