...
BallVolumeProfile::usage = "BallVolumeProfile[g, v] gives the Association r -> |B_r(v)| of cumulative ball volumes over r = 0 .. eccentricity(v). BallVolumeProfile[g, verts] (or All, = BallVolumeProfile[g]) returns one Association per vertex. This is the volume-growth profile underlying WolframHausdorffDimension and WolframRicciCurvature.";
...
WolframRicciCurvature::usage = "WolframRicciCurvature[g, v, {rmin, rmax}] returns the volume-comparison Ricci scalar at vertex v averaged over the radius window; the third argument also accepts a single radius r or All (default). WolframRicciCurvature[g, verts, range] returns a list over the given vertices, and WolframRicciCurvature[g, All, range] (= WolframRicciCurvature[g]) a list over VertexList[g]. Option: \"Dimension\" (Automatic (default) | Integer).";
WolframHausdorffDimension::usage = "WolframHausdorffDimension[g, v, {rmin, rmax}] returns the volume-growth local dimension d(v, r) = (Log V(r+1) - Log V(r)) / (Log(r+1) - Log r), V(r) = |B_r(v)|, at vertex v averaged over the radius window; the third argument also accepts a single radius r or All (default). WolframHausdorffDimension[g, verts, range] returns a list over the given vertices, and WolframHausdorffDimension[g, All, range] (= WolframHausdorffDimension[g]) a list over VertexList[g].";
