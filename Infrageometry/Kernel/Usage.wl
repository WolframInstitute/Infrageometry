...
InfraSubstrateCode::usage = "InfraSubstrateCode[name, size, style] is the code behind InfraSubstrate[name, size, style]: the construction with the style applied to it, as one line of code in a string, which under the same SeedRandom gives the identical graph. The roster line is read off its own definition rather than transcribed, so the printed code cannot drift from the code that runs; the size table and the layout dimension are baked in, and a call to a private helper is replaced by the body it stands for. Takes the options of InfraSubstrate, and Graph options are forwarded as they are by InfraSubstrate.";
