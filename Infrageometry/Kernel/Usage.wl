...
WolframDimension::usage = "WolframDimension[g] returns Association[v -> mean_r d(v, r)] with the volume-growth local dimension d(v, r) = (Log V(r+1) - Log V(r)) / (Log(r+1) - Log r), V(r) = |B_r(v)|, averaged over the per-vertex valid radius range. WolframDimension[g, r] gives a single radius; WolframDimension[g, {rmin, rmax}] gives a window.";
...
InteriorMeshGraph::usage = "InteriorMeshGraph[mr] returns the full 1-skeleton of mr with the 1-cells lying in boundary walls of the top-dimensional cells deleted.";
