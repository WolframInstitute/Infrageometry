...
WolframVolumeLogDifferenceQuotients::usage = "WolframVolumeLogDifferenceQuotients[g, v] gives the volume-growth log-difference quotients q(r) = (Log V(r+1) - Log V(r)) / (Log(r+1) - Log r) at vertex v for r = 1..ecc(v)-1, the discrete d Log V / d Log r at the geometric-mean radius Sqrt[r (r+1)]. WolframVolumeLogDifferenceQuotients[g, verts] returns one list per vertex and WolframVolumeLogDifferenceQuotients[g] (= WolframVolumeLogDifferenceQuotients[g, All]) one per VertexList[g]. Options: \"Volume\" (\"PeeledBall\" (default, reproduces ResourceFunction LogDifferences) | \"Count\" | \"Interior\"), \"Abscissa\" (False (default) | True for {Sqrt[r (r+1)], q} pairs), and \"Aggregation\" (None (default, one list per vertex) | \"MeanAround\" | \"Mean\", which averages the volume profiles over the vertex slot first and returns a single slope-of-mean list; pass a vertex subset to aggregate over just that subset).";
