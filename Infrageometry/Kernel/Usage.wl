...
TessellatedDiskGraph::usage = "TessellatedDiskGraph[{p, q}, r] is the radius-r graph-distance ball cut from the infinite regular {p, q} tessellation of its covering surface (the non-compact companion to TessellationGraph), grown in concentric reflection rings and dispatched by curvature (p-2)(q-2): the Euclidean plane (== 4), the hyperbolic plane (> 4), or the closing-up sphere (< 4). Interior vertices have degree q; VertexCoordinates carry the embedding (2-D in the plane / Poincare disk, 3-D on the unit sphere). TessellatedDiskGraph[{p, q}, {m, n}] is the m x n rectangular Euclidean patch.";
