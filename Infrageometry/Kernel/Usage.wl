...
DimensionCurvatureFit::usage = "DimensionCurvatureFit[q, radii] fits the dimension d and scalar curvature R to the log-difference quotients q paired with their radii by Bishop-Gromov regression on x = r (r + 1), the squared geometric-mean radius Sqrt[r(r+1)], returning <|\"Dimension\" -> d, \"ScalarCurvature\" -> R, \"Window\" -> {rmin, rmax}|>. DimensionCurvatureFit[q, radii, window] restricts the fit to window = {rmin, rmax} | All | Automatic (default, the linear core of the (x, q) scatter). The caller supplies the quotients, so the convention is its choice -- e.g. DimensionCurvatureFit[LogDifferenceQuotients[BallVolumes[g, v, \"Counting\"]], radii] for the index-based counting estimator; Around-valued q (averaging a vertex subset first) carries its spread through the closed-form fit to Around dimension and curvature. Options: \"Probe\" (\"Ball\" (default), intercept d, R = -3 (d + 2) slope | \"Sphere\", intercept d - 1, R = -3 d slope) and \"Dimension\" (Automatic (default) | Integer, pins the intercept).";
