...
BallVolumeProfile::usage = "BallVolumeProfile[g, v] gives the List {V(0), ..., V(ecc(v))} of cumulative ball volumes V(r) = |B_r(v)| (position i is radius i - 1). The optional third argument selects radii: r_Integer returns the scalar V(r) (saturating past eccentricity), {rmin, rmax} the sub-list (default All). BallVolumeProfile[g, verts, range] returns one result per vertex and BallVolumeProfile[g, All, range] (= BallVolumeProfile[g]) one per VertexList[g]. This is the volume-growth profile underlying WolframHausdorffDimension and WolframRicciCurvature.";
CoordinationSequence::usage = "CoordinationSequence[g, v] gives the List {S(0), ..., S(ecc(v))} of sphere sizes S(r) = #{w : d(v, w) == r} (position i is radius i - 1), the discrete derivative of BallVolumeProfile (S(1) is the coordination number; Accumulate recovers BallVolumeProfile). The optional third argument selects radii: r_Integer returns the scalar S(r) (0 past eccentricity), {rmin, rmax} the sub-list (default All). CoordinationSequence[g, verts, range] returns one result per vertex and CoordinationSequence[g, All, range] (= CoordinationSequence[g]) one per VertexList[g].";
...
WolframVolumeLogDifferenceQuotients::usage = "WolframVolumeLogDifferenceQuotients[g, v] gives the volume-growth log-difference quotients q(r) = (Log V(r) - Log V(r-1)) / (Log(r+1) - Log r) at vertex v for r = 1..ecc(v), the discrete d Log V / d Log r; this is ResourceFunction[\"LogDifferences\"] of the ball-volume profile. WolframVolumeLogDifferenceQuotients[g, verts] returns one list per vertex and WolframVolumeLogDifferenceQuotients[g] (= WolframVolumeLogDifferenceQuotients[g, All]) one per VertexList[g]. Options: \"Volume\" (\"Count\" (default) | \"PeeledBall\" | \"Interior\"), \"Abscissa\" (False (default) | True for {Sqrt[r (r+1)], q} pairs at the geometric-mean radius), and \"Aggregation\" (None (default, one list per vertex) | \"MeanAround\" | \"Mean\", which averages the volume profiles over the vertex slot first and returns a single slope-of-mean list; pass a vertex subset to aggregate over just that subset).";
WolframDimensionCurvatureFit::usage = "WolframDimensionCurvatureFit[g, v, window] returns <|\"Dimension\" -> d, \"ScalarCurvature\" -> R, \"Window\" -> {rmin, rmax}|> at vertex v from the Bishop-Gromov regression of the log-difference quotient q(r) on r (r + 1) over the radius window: intercept d, slope -R/(3 (d + 2)). The window is {rmin, rmax}, All, or Automatic (default), which fits on the linear core: the longest radius window where q stays linear within twice the noise floor. WolframDimensionCurvatureFit[g, verts, window] returns a list over the given vertices and WolframDimensionCurvatureFit[g] (= WolframDimensionCurvatureFit[g, All]) one per VertexList[g]. Options: \"Dimension\" (Automatic (default) | Integer, pins the intercept) and \"Volume\" (\"PeeledBall\" (default) | \"Count\" | \"Interior\").";
WolframRicciCurvature::usage = "WolframRicciCurvature[g, v] returns the list of volume-comparison Ricci scalars R(v, r) = 6 (d + 2) / r^2 (1 - v(r) / V_E(d, r)) over r = 1..ecc(v), with V_E(d, r) the Euclidean d-ball volume. WolframRicciCurvature[g, verts] returns one list per vertex and WolframRicciCurvature[g] (= WolframRicciCurvature[g, All]) one per VertexList[g]. Options: \"Dimension\" (Automatic (default), fitted via WolframDimensionCurvatureFit | Integer) and \"Volume\" (\"PeeledBall\" (default) | \"Count\" | \"Interior\").";
WolframHausdorffDimension::usage = "WolframHausdorffDimension[g, v, window] returns the volume-growth dimension at vertex v: the fitted intercept of the Bishop-Gromov regression of WolframDimensionCurvatureFit over the radius window ({rmin, rmax}, All, or Automatic (default), the linear core). WolframHausdorffDimension[g, verts, window] returns a list over the given vertices and WolframHausdorffDimension[g] (= WolframHausdorffDimension[g, All]) one per VertexList[g]. Option \"Volume\" (\"PeeledBall\" (default) | \"Count\" | \"Interior\") picks the ball-volume convention.";
...
FindBallCover::usage = "FindBallCover[g, r] returns a minimum r-ball cover of g: a smallest set of centres whose radius-r balls cover every vertex (a minimum r-dominating set). FindBallCover[g, r, targets] covers only the given vertex subset (centres still chosen from all of g).";
