...
EccentricitySubgraph::usage = "EccentricitySubgraph[g, {lo, hi}] gives the induced subgraph on { v : lo <= e(v) <= hi } with e the eccentricity in hops; a bare k means e(v) <= k, and the default band is everything. Vertex labels and coordinates are g's, so a construction made on the band draws on g. Absolute cousin of RelativeEccentricitySubgraph.";
...
InfraSubstrateStyle::usage = "InfraSubstrateStyle[name] is the Graph option list of the named substrate backdrop style -- \"Gray\", \"GrayOpaque\" or \"GrayFaint\" (StandardGray edges on an opacity ladder, faintly filled outlined vertex disks at a scaled 0.013 | 0.009 | 0.006 of the coordinate diagonal, so one style draws one dot on every graph), \"Default\" for none; the size names \"Small\" | \"Medium\" | \"Large\" are aliases for \"Gray\" | \"GrayOpaque\" | \"GrayFaint\". Splice it into any graph construction: Graph[g, Sequence @@ InfraSubstrateStyle[\"Medium\"]] -- the way to draw a hand-built object (a Wolfram-model graph in particular) exactly like the roster substrates. InfraSubstrateStyle[] lists the style names."

InfraSubstrate::usage = "InfraSubstrate[name, size] is the named example substrate at size \"Small\", \"Medium\" or \"Large\", or at a raw spec (a cell measure, a radius, grid dimensions, a generation count); generation is seeded per (name, size, inflation), so repeated calls return the same graph. Each substrate is one explicit definition in Kernel/InfraSubstrate.wl, with its size table and its exceptions (an interior strip, a kept embedding) written into the definition. A Wolfram-model universe is named by its Registry of Notable Universes number, as in InfraSubstrate[\"wm6655\", 11], and any of the 947 registry entries resolves through ResourceFunction[\"WolframModelData\"]. InfraSubstrate[name, size, style] overrides the ambient style, which defaults to the InfraSubstrateStyle gray of that size since a substrate is a backdrop; pass \"Default\" for none. A substrate is bare combinatorics by default -- a stored embedding is discarded and a spring layout of its own dimension places the vertices; option \"KeepCoordinates\" -> True draws the substrate where it lives instead. Any substrate inflates: option \"Inflate\" -> amount grows a fiber of that many extra vertices over every vertex through InflateGraph (a constant or a {min, max} range), and \"Inflate\" -> {opts} passes InflateGraph its full option list. InfraSubstrate[] lists the roster classified by what a substrate models (\"OpenManifold\" -- boundaryless patches, open subsets delivered with the rim contour removed; \"ClosedManifold\" -- compact tessellated surfaces; \"Fractal\" -- self-similar, Hausdorff dimension between the integers; \"Exotic\" -- graphs with no manifold model and no scaling law; \"WolframModel\"); InfraSubstrate[All] is the flat name list; InfraSubstrate[name] is the \"Medium\" size. Graph options are forwarded.";

InfraSubstrateCode::usage = "InfraSubstrateCode[name, size, style] is the code behind InfraSubstrate[name, size, style]: one held Graph call -- the construction with the style applied to it -- that ReleaseHold under the same SeedRandom evaluates to the identical graph. It is HoldForm, so an output cell shows it as typeset code. The roster line is read off its own definition rather than transcribed, so the printed code cannot drift from the code that runs; the size table, the layout dimension and the resolved style are baked in, and a call to a private helper is replaced by the body it stands for. Takes the options of InfraSubstrate, and Graph options are forwarded as they are by InfraSubstrate.";
